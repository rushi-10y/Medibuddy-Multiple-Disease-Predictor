...
def diabetesPredictPage():
    if request.method == 'POST':
        try:
            # Get user input from the form fields (make sure these match the input names in your 'diabetes.html')
            age = float(request.form['age'])
            bmi = float(request.form['bmi'])
            glucose = float(request.form['glucose'])
            insulin = float(request.form['insulin'])
            # You can add more features like blood pressure, skin thickness, etc.
            
            # Prepare the input data (reshape if needed for your model)
            input_data = np.array([[age, bmi, glucose, insulin]])  # Adjust this with all the features you're using
            
            # Load your trained model (make sure to specify the correct model path)
            model = tf.keras.models.load_model("models/diabetes_model.h5")
            
            # Predict using the model
            pred = model.predict(input_data)
            
            # You can customize the threshold based on your model's output
            if pred[0] > 0.5:  # If it's a binary classification model
                result = "Positive for Diabetes"
            else:
                result = "Negative for Diabetes"
            
        except Exception as e:
            # Handle errors gracefully (e.g., missing fields or invalid input)
            message = f"Error: {str(e)}. Please provide valid inputs."
            return render_template('diabetes.html', message=message)
    
    return render_template('diabetes_predict.html', pred=result)
